...
Validator.isFile = (selector, message) => {
  return {
    ...
    testing: (value) => {
      if (value) {
        const regex =
          /([a-zA-Z0-9\s_\\.\-:])+(.doc|.docx|.pdf)$/; // regex kiểm tra định dạng file
        return regex.test(value) ?
          undefined :
...
Validator.isSizeFile = (selector, max = 3, messageMin, messageMax) => {
  return {
    selector,
    testing: (value) => {
      ...
      if (!file) {
        ...
        if (fileSizeInBytes < 1) {
          return messageMin || `Minimum file size 1KB.`;
        }
        if (fileSizeInBytes > 1024 * 1024 * max) {
          return messageMax || `The file size must not exceed ${max}.`;
        }
      }
    }
